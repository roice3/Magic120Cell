...
HGLRC CGlInit::initializeGL( HDC hContext ) 
{
	...
	if( ! wglMakeCurrent( hContext, hrc ) )
		displayErrorMessage( "wglMakeCurrent" );
...
BOOL CGlInit::setupPixelFormat( HDC hContext ) 
{
	...
	// Do some checking.
	{
		...
		if( ! acceleratedFlag )
		{
			...
			{
				assert( false );
				MessageBoxA( NULL, "WARNING! The program may not be running in graphics accelerated mode.", "MagicCube5D", MB_ICONWARNING );
			}
		}
